...
def quiz_post(quiz_id):
    ...
    # Use bind parameters to safely pass the variables
    db.session.execute(
        text('UPDATE submits_quiz SET marks = :marks WHERE student_id = :student_id and quiz_id = :quiz_id'),
        {'marks': marks, 'student_id': current_user.student.id, 'quiz_id': quiz_id}
    )
    db.session.commit()
...
def view_performance():
    ...
    # Use bind parameters to safely pass the variables
    quiz_submitted_query = list(db.session.execute(
        text('SELECT * FROM submits_quiz WHERE student_id = :student_id'),
    ...
    for quiz in quiz_submitted_query:
        ...
        all_marks = list(db.session.execute(
            text('SELECT marks FROM submits_quiz WHERE quiz_id = :quiz_id'),
            {'quiz_id': quiz[1]}
        ))
        all_marks = [x[0] if x[0] is not None else 0 for x in all_marks]  # Handle None values
        quiz_submitted.append(dict(quiz_title=quiz_title, marks=quiz[3] if quiz[3] is not None else 0, all_marks=all_marks))
...
def view_result():
    ...
    for quiz in quiz_submitted_query:
        ...
        quiz_submitted.append(dict(title=quiz_title, marks=quiz[3] if quiz[3] is not None else 0, total_marks=total_marks))
